...
interface ChatDialogProps {
  ...
  listingTitle: string;
  sellerId: string;
  sellerPhone?: string; // Optional - only shown if seller allows WhatsApp contact
}
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="flex h-[80vh] max-h-[600px] flex-col p-0 sm:max-w-md">
        ...
        {/* WhatsApp bridge - only if seller allows it */}
        {sellerPhone ? (
          ...
          <div className="border-t bg-muted/50 p-3">
            <p className="text-center text-sm text-muted-foreground">
              WhatsApp no habilitado por el oferente. Continuá por chat.
            </p>
          </div>
        )}
